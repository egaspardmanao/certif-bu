...
import { Plus, Trash2, Flag, RotateCcw, Users, Edit2, Search } from 'lucide-react'
...
function CarteProjet({ projet, onClick }) {
  ...
  return (
    <button onClick={() => onClick(projet)}
      className={`card p-4 text-left w-full hover:border-slate-300 transition-all group ${projet.est_special ? 'border-pink-300' : ''} ${projet.termine ? 'opacity-60' : ''}`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="font-display font-bold text-base text-slate-900 leading-tight">{projet.nom}</div>
...
function PanneauProjet({ projet, allConsultants, toutesLesPersonnes, onClose, onUpdated, showToast }) {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-sm bg-white border-l border-slate-200 flex flex-col shadow-2xl overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-slate-200">
          <h2 className="font-display font-bold text-xl text-slate-900">{projet.nom}</h2>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-900 p-1">✕</button>
        </div>
        <div className="flex-1 p-5 space-y-5">
              ...
              {consultantsDuProjet.map((c, i) => {
                ...
                return (
                  <div key={c.id} className="flex items-center gap-2 group">
                    ...
                    <span className="flex-1 text-sm text-slate-900">{c.prenom} {c.nom}</span>
          ...
          <div className="border-t border-slate-200 pt-4 space-y-2">
            ...
            {!projet.termine ? (
              <button onClick={handleTerminer} className="btn-ghost w-full flex items-center justify-center gap-2 text-sm text-orange-600 border-orange-200">
                <Flag size={14} /> Terminer le projet
              </button>
            ) : (
              <button onClick={handleReprendre} className="btn-ghost w-full flex items-center justify-center gap-2 text-sm text-emerald-600 border-emerald-200">
...
export default function Projets() {
  ...
  const matches = p => p.nom.toLowerCase().includes(search.toLowerCase())
  const speciaux  = projets.filter(p => p.est_special && !p.termine && matches(p))
  const actifs    = projets.filter(p => !p.est_special && !p.termine && matches(p)).sort((a, b) => (b.missions?.length ?? 0) - (a.missions?.length ?? 0))
  const termines  = projets.filter(p => p.termine && matches(p))
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <h3 className="font-display font-bold text-lg text-slate-900 flex-1">Projets actifs</h3>
        <div className="relative flex-1 min-w-48">
          <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
          <input className="input pl-9" placeholder="Rechercher un projet…" value={search}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-8">
        ...
        {actifs.length === 0 && (
          <div className="col-span-3 card p-8 text-center text-slate-500">
            {search ? 'Aucun projet ne correspond à la recherche.' : 'Aucun projet actif.'}
          </div>
        )}
      </div>
